...
class HttpConsumer extends ApiConsumer {
  ...
  @override
  Future<dynamic> patch(String path, {dynamic data, Map<String, dynamic>? queryParameters, bool isFormData = false}) async {
    try {
      final uri = Uri.parse('$baseUrl$path').replace(queryParameters: queryParameters);
      final headers = {
        'Content-Type': isFormData ? 'multipart/form-data' : 'application/json',
  ...
  Future<dynamic> delete(String path, {Object? data, Map<String, dynamic>? queryParameters}) async {
    try {
      final uri = Uri.parse('$baseUrl$path').replace(queryParameters: queryParameters);
      final response = await http.delete(uri, body: data != null ? json.encode(data) : null);
      handleHttpResponse(response);
      return _tryDecode(response.body);
    } catch (e) {
      handleHttpException(e);
