...
import 'package:teriak/features/purchase_order/delete_purchase_order/presentation/controller/delete_supplier_controller.dart';
...
class _PurchaseOrderDetailState extends State<PurchaseOrderDetail> {
  ...
  final deleteController = Get.put(DeletePurchaseOrderController());
        ...
        IconButton(
          ...
          onPressed: () {
            if (controller.purchaseOrder.value?.status == "PENDING") {
              ...
              Get.snackbar(
                  '', "The order cannot be modified in its current state".tr,
        ...
        IconButton(
          ...
          onPressed: () {
            if (controller.purchaseOrder.value?.status == "PENDING") {
              ...
              Get.snackbar(
                  '', "The order cannot be deleted in its current state".tr,
                  backgroundColor: Colors.red, colorText: Colors.white);
            }
          },
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            _buildInfoRow(
              'Total Amount'.tr,
              _formatAmount(order.total?.toDouble() ?? 0.0, order.currency),
              Icons.attach_money,
