...
import 'package:teriak/features/purchase/edit_purchase_order/data/datasources/edit_purchase_remote_data_source.dart';
import 'package:teriak/features/purchase/edit_purchase_order/data/repositories/edit_purchase_repository_impl.dart';
import 'package:teriak/features/purchase/edit_purchase_order/domain/usecases/put_edit_purchase.dart';
...
class EditPurchaseOrderController extends GetxController {
  ...
  void updateOrderItemPrice(int index, double newPrice) {
    if (index >= 0 && index < orderItems.length && isPharmacist.value) {
      final item = orderItems[index];
  ...
  Future<void> updatePurchaseOrder() async {
    ...
    try {
      ...
      result.fold(
        (failure) {
          print(
              'EditPurchaseOrderController: Update failed: ${failure.errMessage}');
          Get.snackbar(
            'Error'.tr,
